...
人机验证会拦截 MetaWeblog 客户端、第三方 App 等通过 XML-RPC 调用的合法机器人(它们无法完成浏览器工作量证明)。若需要这类接入:

1. 为机器人创建**专用账号**,遵循最小权限(通常 `contributor` 及以下即可,不要使用管理员账号),并设置**强密码**;
2. 在插件设置中启用「XML-RPC 豁免」,把该账号的登录用户名填入豁免名单(多个用逗号分隔)。

豁免仅对名单内用户的 XML-RPC 调用生效,其网页端操作照常需要验证;名单外账号的 XML-RPC 调用一律拦截。另请注意:Typecho 内置的「评论反垃圾保护」会在核心层独立拦截 XML-RPC 评论(与插件无关),需要机器人评论能力时请在 设置→讨论 中关闭它。
...
| XML-RPC 豁免 | 允许名单内用户经 XML-RPC 调用时免验证(供合法机器人),务必为其设置强密码并限制权限 |
| XML-RPC 豁免名单 | 免验证的登录用户名列表,仅对 XML-RPC 调用生效 |
